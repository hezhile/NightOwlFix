export default {
  async fetch(request, env, ctx) {
    ...
    try {
        ...
        const behaviorPart =
          hour !== null
            ? (hour >= 23
                ...
                : '未到23:00：用关心的语气询问白天的事情是否已完成？是否准备好进行睡前仪式？如果用户尚未完成，给1–2条可行的快速收尾建议（例如列出3项优先任务、设定5分钟计时器），并给1–2条简单的睡前准备建议（例如关灯、关屏、深呼吸）。')
      ...
      const answer = await env.AI.run('@cf/meta/llama-3.2-1b-instruct', aiOptions);
